...
WebUI.waitForElementPresent(findTestObject('PolicyCenter/PolicyCenterObjects/PC_NewSubmissions_Dropdown_DefaultBaseState'), 
    45, FailureHandling.OPTIONAL)

String tmpState = State

if (tmpState.equalsIgnoreCase('Illinois')) {
    WebUI.selectOptionByValue(findTestObject('PolicyCenter/PolicyCenterObjects/PC_NewSubmissions_Dropdown_DefaultBaseState'), 
        'IL', true, FailureHandling.OPTIONAL)
} else {
    WebUI.waitForElementPresent(findTestObject('PolicyCenter/PolicyCenterObjects/PC_NewSubmissions_Button_LOB_Select_WC'), 
        10)

    WebUI.selectOptionByLabel(findTestObject('PolicyCenter/PolicyCenterObjects/PC_NewSubmissions_Dropdown_DefaultBaseState'), 
        State, true, FailureHandling.STOP_ON_FAILURE)
}
...
if (lob.contains('Workers')) {
    ...
    GlobalVariable.LOB = 'Workers\' Compensation'
